...
$footer_class    = Options::$options['post_tag'] && has_tag() && Options::$options['post_social_icon'] && class_exists( 'Radius_Directory_Core' ) ? 'col-md-6 col-sm-12 col-12'
	: 'col-md-12 col-sm-12 col-12';
$has_post_footer = ( Options::$options['post_tag'] && has_tag() ) || ( Options::$options['post_social_icon'] && class_exists( 'Radius_Directory_Core' ) );
$has_post_social = ( class_exists( 'Radius_Directory_Core' ) && Options::$options['post_social_icon'] );
?>
					...
					<?php if ( $has_post_footer ): ?>
                        <div class="post-footer <?php echo esc_attr( $has_post_social ? '' : 'has-no-share' ) ?>">
                            <div class="row align-items-center">
								...
								<?php endif; ?>
	                            <?php if ( class_exists( 'Radius_Directory_Core' ) && Options::$options['post_social_icon'] ): ?>
                                    <div class="<?php echo esc_attr( $footer_class ); ?>">
                                        <!-- Social Share -->
                                        <?php Radius_Directory_Core::social_share(); ?>
                                    </div>
	                            <?php endif; ?>
                            </div>
                        </div>
