...
class Scripts {
	...
	public function fonts_url() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'radius-directory' ) ) {
	...
	public function register_scripts() {
		...
		// Google fonts
		wp_register_style( 'radius-directory-gfonts', $this->fonts_url(), [], $this->version );
		// Style
		...
		wp_register_style( 'radius-directory-main', Helper::get_maybe_rtl_css( 'main' ), [], $this->version );
		if ( is_rtl() ) {
			wp_register_style( 'radius-directory-rtl', Helper::get_css( 'rtl' ), [], $this->version );
		}

		// Script
		wp_register_script( 'bootstrap', Helper::get_js( 'bootstrap.bundle.min' ), [ 'jquery' ], $this->version, true );
		wp_register_script( 'radius-directory-main', Helper::get_js( 'main' ), [ 'jquery' ], $this->version, true );
	}

	public function enqueue_scripts() {
		/*------CSS--------*/
		wp_enqueue_style( 'radius-directory-gfonts' );
		wp_enqueue_style( 'bootstrap' );
		wp_enqueue_style( 'font-awesome' );
		wp_enqueue_style( 'radius-directory-main' );
		if ( is_rtl() ) {
			wp_enqueue_style( 'radius-directory-rtl' );
		}
		$this->dynamic_style();
		/*-------JS--------*/
		...
		wp_enqueue_script( 'radius-directory-main' );
		$this->localized_scripts(); // Localization
	}

	public function register_gutenberg_scripts() {
		wp_register_style( 'radius-directory-gfonts', $this->fonts_url(), [], $this->version );
		wp_register_style( 'radius-directory-gutenberg', Helper::get_maybe_rtl_css( 'gutenberg' ), [], $this->version );
	}

	public function enqueue_gutenberg_scripts() {
		wp_enqueue_style( 'radius-directory-gfonts' );
		wp_enqueue_style( 'radius-directory-gutenberg' );
		ob_start();
		...
		wp_add_inline_style( 'radius-directory-gutenberg', $css );
	}
	...
	private function localized_scripts() {
		...
		$localize_data = apply_filters( 'radius_directory_localized_data', $localize_data );

		wp_localize_script( 'radius-directory-main', 'CLClassified', $localize_data );
	}
	...
	private function dynamic_style() {
		...
		wp_register_style( 'radius-directory-dynamic', false );
		wp_enqueue_style( 'radius-directory-dynamic' );
		wp_add_inline_style( 'radius-directory-dynamic', $dynamic_css );
	}
