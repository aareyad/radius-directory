...
<?php if ( have_comments() ): ?>
	...
	$comments_text   = $comments_number == 1 ? esc_html__( 'Comment', 'radius-directory' ) : esc_html__( 'Comments', 'radius-directory' );
	...
	$comment_args    = [
		'callback'    => 'RadiusTheme\RadiusDirectory\Helper::comments_callback',
		'reply_text'  => esc_html__( 'Reply', 'radius-directory' ),
		'avatar_size' => 70,
	];
	?>
        ...
        <div class="main-content">
            <div class="comments-area">
                ...
                <?php if ( ! comments_open() ) : ?>
                    <div class="comments-closed"><?php esc_html_e( 'Comments are closed.', 'radius-directory' ); ?></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
$comment_form_fields = [
	'author' =>
		...
		. '" placeholder="' . esc_attr__( 'Name', 'radius-directory' ) . ( $rdtheme_req ? ' *' : '' ) . '" class="form-control"' . $rdtheme_aria_req . '></div>',

	'email' =>
		'<div class="col-lg-6 form-group"><input id="email" name="email" type="email" value="' . esc_attr( $rdtheme_commenter['comment_author_email'] )
		. '" class="form-control" placeholder="' . esc_attr__( 'Email', 'radius-directory' ) . ( $rdtheme_req ? ' *' : '' ) . '"' . $rdtheme_aria_req . '></div></div>',
];

$comment_form_args = [
	'class_submit'  => 'submit btn-send',
	'submit_field'  => '<div class="form-group submit-button">%1$s %2$s</div>',
	'comment_field' => '<div class="form-group"><textarea id="comment" name="comment" required placeholder="' . esc_attr__( 'Comment *', 'radius-directory' )
...
<?php if ( comments_open() ): ?>
    ...
    <div class="site-content-block comment-reply-block">
        <div class="main-title-block">
            <h3 class="main-title"><?php esc_html_e( 'Leave a Comment', 'radius-directory' ); ?></h3>
        </div>
        <div class="main-content">
